...
struct Svm : public FaceRecognizer
{
    ...
    Svm(int pre = 0, int use_uni=0, double degree = 0.5,double gamma = 0.8,double coef0 = 0,double C = 0.99, double nu = 0.2, double p = 0.5) 
        : preprocessing(pre)
    {
	    param.kernelType = SVM :: POLY ; // CvSVM :: RBF , CvSVM :: LINEAR...
...
public:
    ...
    virtual void predict(InputArray src, int& label, double & minDist) const    
    {
        Mat m = src.getMat();
        Mat q = preproc(m);
        float r = svm->predict ( q.reshape(1,1) );
...
Ptr<FaceRecognizer> createSvmFaceRecognizer(int pre, int use_uni=0)
{
    return makePtr<Svm>(pre,use_uni);
}
